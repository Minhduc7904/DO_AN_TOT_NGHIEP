...
Với yêu cầu lập/chia task tuần hoặc hỏi công việc tuần, đọc `agent-resources/skills/weekly-task-planning/SKILL.md`. Với yêu cầu xác nhận task đã hoàn thành, đọc `agent-resources/skills/task-completion-recording/SKILL.md` và xác nhận sản phẩm cùng từng DoD trước khi chuyển task sang `Chờ review`. Với review code/PR, đọc `agent-resources/skills/task-code-review/SKILL.md`.

Trước khi tạo branch, sửa code/tài liệu hoặc mở pull request, đọc `docs/processed/rules/naming-rules.md`, `docs/processed/rules/git-and-pull-request-rules.md` và `docs/processed/guides/git-workflow.md`. Mỗi task cần một branch và PR trước khi chờ review; chỉ merge sau khi review đạt và task ở `Hoàn thành`; dùng `.github/pull_request_template.md`.
