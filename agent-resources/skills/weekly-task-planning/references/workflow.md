...
- Mỗi task phải có PR vào `main`. Card task và output giữ URL/số PR. Người phụ trách hoàn tất phần việc sẽ ghi URL/số PR, chuyển `Chờ review` và commit/push transition này vào chính PR head trước review; sau `APPROVED`, chính người phụ trách dùng `task-completion-recording` để finalization và chuyển `Hoàn thành` trên branch/PR trước merge. Chỉ khi commit đó vào `main` thì task mới canonically hoàn thành. Requirement commit/push không tự cấp quyền Git write cho agent. PR tuân thủ [vòng đời task canonical](../../../../docs/processed/rules/git-and-pull-request-rules.md#vòng-đời-task-canonical) và template PR canonical.
...
- Không đánh dấu hoàn thành chỉ vì plan nói cần làm. Chỉ người phụ trách, qua `task-completion-recording` sau `APPROVED`, mới finalization trạng thái trên branch/PR; `main` sau merge là nguồn trạng thái project-wide canonical.
