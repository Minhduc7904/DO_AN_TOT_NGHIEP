...
- Skill này không tự quyết định task đã hoàn thành, không tự merge và không đổi trạng thái task. Sau khi feedback đã được xử lý, reviewer kiểm tra tập trung diff xử lý feedback và gửi `APPROVED`; không cần review lại toàn bộ PR. Chỉ sau khi approval hợp lệ xuất hiện trên GitHub, người phụ trách dùng `task-completion-recording` để ghi `Hoàn thành` và push metadata vào chính PR trước khi tự merge theo [vòng đời task canonical](../../../../docs/processed/rules/git-and-pull-request-rules.md#vòng-đời-task-canonical). Skill này không được dùng để đóng task hoặc xử lý bookkeeping hậu-merge.
