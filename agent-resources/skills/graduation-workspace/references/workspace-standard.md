...
- Thứ tự review, merge và ghi nhận hoàn thành dùng duy nhất [vòng đời task canonical](../../../../docs/processed/rules/git-and-pull-request-rules.md#vòng-đời-task-canonical). Reviewer thực hiện một vòng review đầy đủ; nếu có feedback, người phụ trách sửa và reviewer kiểm tra tập trung phần sửa trước khi gửi `APPROVED`. Review chưa tự là **Hoàn thành**; chỉ người phụ trách dùng `task-completion-recording` để finalization, chuyển trạng thái trên branch/PR và yêu cầu/thực hiện merge.
...
- Bảng task chỉ là bản phân công. Trên branch task, card chỉ được chuyển sang **Hoàn thành** khi mọi DoD có bằng chứng, đã có link đến `workspace/<owner>/.../input/task-input.md`, `output/task-output.md`, URL PR và GitHub ghi nhận `APPROVED` hợp lệ từ thành viên còn lại. Project-wide chỉ coi card hoàn thành sau khi người phụ trách merge commit đó vào nhánh canonical.
...
Với mọi task mới, output phải ghi link pull request, reviewer và verdict review lấy từ GitHub. Mô tả PR phải tuân thủ `docs/processed/rules/git-and-pull-request-rules.md`; không ghi trạng thái mutable của PR hoặc merge metadata có thể suy ra từ Git/GitHub.

Chỉ đặt trạng thái **Hoàn thành** trên branch task theo [vòng đời task canonical](../../../../docs/processed/rules/git-and-pull-request-rules.md#vòng-đời-task-canonical) khi tất cả DoD đạt, sản phẩm mở được, mọi feedback blocking đã được xử lý và GitHub ghi nhận `APPROVED` hợp lệ từ thành viên còn lại; sau đó người phụ trách cập nhật completion record trong chính PR. Sau khi chính người phụ trách merge commit đó vào nhánh canonical, task mới canonically hoàn thành. Nếu phần việc đã xong nhưng chưa review/approve, dùng **Chờ review**; `Chờ xử lý` chỉ dùng khi có blocker/dependency thật, không dùng chỉ vì đang chờ merge.
