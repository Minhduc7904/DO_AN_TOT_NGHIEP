...
5. **Đồng bộ JSON và timeline.** Chạy `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\sync-plan-json-and-timeline.ps1` từ repository root. Nếu script lỗi hoặc thiếu runtime, cảnh báo. Khi đúng người phụ trách xác nhận tiếp tục, được commit/push Markdown theo ngoại lệ, ghi rõ timeline đang chờ đồng bộ và không sửa tay đầu ra.
