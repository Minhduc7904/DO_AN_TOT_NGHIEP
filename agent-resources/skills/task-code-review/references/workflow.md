...
4. Lấy card task ở **commit hiện tại của PR head/task branch trên remote**. Review ban đầu chỉ hợp lệ khi bản card này có URL/số PR, đúng nhánh và trạng thái `Chờ review`. Không lấy `main` hoặc working tree local làm nguồn readiness: `main` chỉ là nguồn trạng thái project-wide canonical. Khi kiểm tra phần sửa hoặc re-approval do commit mới, card ở PR head có thể đã là `Hoàn thành`; chỉ kiểm tra diff mới kể từ review trước, không review lại toàn bộ PR. Nếu không xác định/đọc được đúng PR head, báo rõ chưa thể review; không tự suy diễn từ `main`.
...
4. Với `CHANGES_REQUESTED`, người phụ trách xử lý bằng `pr-review-response`. Sau khi thay đổi đã push và từng thread có reply, reviewer chỉ kiểm tra tập trung diff xử lý feedback; nếu mọi blocking issue đã được giải quyết thì gửi verdict GitHub `APPROVED`, không review lại toàn bộ PR.
5. `APPROVED` nghĩa là substantive work đủ điều kiện finalization, không có nghĩa PR đã merge hoặc task đã canonically hoàn thành. Sau verdict này, người phụ trách dùng `task-completion-recording` để ghi metadata `Hoàn thành` trên chính branch/PR **trước merge** theo [vòng đời task canonical](../../../../docs/processed/rules/git-and-pull-request-rules.md#vòng-đời-task-canonical).
6. Nếu commit mới làm GitHub dismiss/stale approval, reviewer chỉ kiểm tra diff mới kể từ lần review trước và re-approve khi hợp lệ. Reviewer không cập nhật completion record hoặc trạng thái task thay người phụ trách.
7. Reviewer không merge PR. Chỉ người phụ trách task được yêu cầu hoặc thực hiện merge task của mình sau finalization.
