...
| [task-06_verify-enroll-to-submit-workflow](task-06_verify-enroll-to-submit-workflow.md) | Kiểm chứng E2E login → enroll → nộp bài qua Gateway | Bách | Đức | Trung bình | Hoàn thành |
...
- [#30](https://github.com/Minhduc7904/DO_AN_TOT_NGHIEP/pull/30) — `task-06_verify-enroll-to-submit-workflow`, base `main`; finalization trên branch theo ngoại lệ do Bách chỉ thị trực tiếp ngày 26/09/2026 sau review kỹ thuật ngoài GitHub. Không có GitHub `APPROVED`; branch protection vẫn có thể chặn merge.
...
- [x] Luồng `login → enroll → nộp bài` chạy qua Gateway với PostgreSQL thực; Enrollment và Submission có persistence riêng (`enrollment_db`, `submission_db` nếu cần).
- [x] Ít nhất một HTTP contract test cho Submission↔Course và Submission↔Enrollment.
- [x] Storage mock có dependency identity ổn định, điều khiển được latency/error và tạo outbound/dependency span riêng biệt.
- [x] Trace xuyên Gateway, Enrollment, Submission và các dependency giữ W3C context; resilience cơ bản (timeout/retry) khi gọi Course/Enrollment được kiểm tra.
- [x] Mỗi task có PR riêng, bằng chứng DoD và trạng thái đã finalization trên branch; riêng task-06 dùng review ngoài GitHub theo ngoại lệ Bách xác nhận.
