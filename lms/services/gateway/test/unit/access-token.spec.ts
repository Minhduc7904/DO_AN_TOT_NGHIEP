...
describe('verifyAccessToken', () => {
  ...
  it('rejects altered signatures, expired tokens, and missing principal claims', () => {
    ...
    expect(verifyAccessToken(createToken(validClaims, `${secret}-other`), secret, 1_800_000_000_000)).toBeNull();
    expect(verifyAccessToken(createToken({ ...validClaims, exp: 1_700_000_000 }), secret, 1_800_000_000_000)).toBeNull();
    expect(verifyAccessToken(createToken({ exp: 1_900_000_000, role: 'student' }), secret, 1_800_000_000_000)).toBeNull();
  });
});
